...
> Capstone project (Mar 2026 – Dec 2026) · **Team — sole AI developer** · Live service: [inpose.co](https://inpose.co/)
...
The core ML system — semantic feed clustering, real-time inference, and user–cluster preference mapping — is **built and running in production**. The recommendation layer that turns these signals into personalized job-posting and feed suggestions is the next milestone of this capstone (Mar–Dec 2026).

| Component | Status |
|-----------|--------|
| Feed image clustering (CLIP + K-Means) | ✅ Deployed |
| Real-time cluster inference (FastAPI) | ✅ Deployed |
| User–cluster preference mapping | ✅ Deployed |
| Recommendation layer (job posts + feed) | Roadmap |
...
Sole AI developer on the capstone team. Designed and implemented the full ML system **single-handedly** — embedding extraction, the algorithm-selection study, the clustering pipeline, the real-time inference server (FastAPI), the user-affinity mapping module, and its production deployment — with the recommendation layer as the next milestone.
